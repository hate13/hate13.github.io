...
2. TBufferedTransport 和 TFramedTransport 有什么区别？
   ...
   他们都是对其他 Transport 的装饰，TBufferedTransport 的作用是维护一个 buffer，减少底层 Transport 的读写次数。比如在 Golang 中，[TSocket](https://github.com/apache/thrift/blob/master/lib/go/thrift/socket.go) 每次读写都会调用底层 net.Conn 进行操作，效率比较低，可以在上面套一层 [TBufferedTransport](https://github.com/apache/thrift/blob/master/lib/go/thrift/buffered_transport.go)，减少读写次数。
...
4. TNonBlockingServer 和 TTheadedSelectorServer 是怎么实现的？有什么区别？
...
5. 为什么异步调用必须使用 TFramedTransport？
...
6. Golang 和 Java 的主要区别？为什么 Golang 里不需要那么多乱七八糟的 TServer 实现？
