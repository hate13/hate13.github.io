...
最近需要做一个流式任务 JOIN 外部 RPC 数据的需求，基于性能考虑，需要使用“异步”的方式调用，便深入看了下。
...
比如在 Golang 中，[TSocket](https://github.com/apache/thrift/blob/0.9.3/lib/go/thrift/socket.go) 每次读写都会调用底层 net.Conn 进行操作，效率比较低，可以在上面套一层 [TBufferedTransport](https://github.com/apache/thrift/blob/0.9.3/lib/go/thrift/buffered_transport.go)，减少读写次数。
...
实现比较简单，内部有一个线程池，每来一个请求，就交给线程池进行执行，源码：[TThreadPoolServer.java](https://github.com/apache/thrift/blob/0.9.3/lib/java/src/org/apache/thrift/server/TThreadPoolServer.java)
...
多扯一句，这里说的异步调用是指用户视角，内部还是使用的 Java NIO 实现（即上面说到的 TNonBlockingServer 和 TTheadedSelectorServer），在网络编程中不是真正的异步，只能算同步非阻塞。
